...
class Stats(Cog):
    ...
    async def stats(self, ctx: ApplicationContext):
        ...
        embed.add_field(name="New Subscriptions:", value=f"`{self.bot.new_subscriptions}`", inline=True)
        embed.add_field(name="Deleted Subscriptions:", value=f"`{self.bot.del_subscriptions}`", inline=False)
        embed.add_field(name="Queued Events:", value=f"`{len(Cozyfications.QUEUE)}`", inline=True)
